...
abstract class Coded implements \marvin255\bxmigrate\IMigrate
{
     ...
     */
    protected function HLCreate(array $data, $deleteIfExists = false)
    {
        if (empty($data['NAME'])) throw new \Exception('You must set hl NAME');
        if (empty($data['TABLE_NAME'])) throw new \Exception('You must set hl TABLE_NAME');
        $fire = false;
        if ($this->HLGetIdByCode($data['NAME'])) {
            if ($deleteIfExists) {
                $this->HLDelete($data['NAME']);
                $fire = true;
            }
        } else {
            $fire = true;
        ...
        if ($fire) {
            ...
            if ($result->isSuccess()) {
                BXClearCache(true, '/');
                echo "Add {$data['NAME']} highload block\r\n";
            } else {
                throw new \Exception("Can't create {$data['NAME']} highload block");
            }
        }
    }
    ...
    {
        if (empty($data['NAME'])) throw new \Exception('You must set NAME');
        if ($id = $this->HLGetIdByCode($data['NAME'])) {
            unset($data['NAME']);
            $result = \Bitrix\Highloadblock\HighloadBlockTable::update($id, $data);
            if ($res->isSuccess()) {
                BXClearCache(true, '/');
                echo "Update {$data['NAME']} highload block\r\n";
            } else {
                throw new \Exception("Can't update {$data['NAME']} highload block");
            }
            BXClearCache(true, '/');
        } else {
            throw new \Exception("Can't update {$data['NAME']} highload block");
    ...
    protected function HLDelete($entity)
    {
        ...
        if ($id) {
            ...
            if ($res->isSuccess()) {
                BXClearCache(true, '/');
                echo "Delete highload block {$entity}\r\n";
            } else {
